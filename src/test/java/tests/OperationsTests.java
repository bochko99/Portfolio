...
public class OperationsTests extends MobileTest {
    ...
    public void testGetOperations() {
        service().auth().get(EndPoints.operations);
    }
    ...
    public void testOperationsIdComment() {
        ...
        OperationModel[] operations = service().auth().queryParam("take", 1).get(EndPoints.operations).as(OperationModel[].class);
        String id = operations[0].getId();
        service().auth().pathParam("id", id).body(model).post(EndPoints.operations_id_comment);
        service().auth().queryParam("take", 5).get(EndPoints.operations).then()
    ...
    public void testPostOperationsRead() {
        service().auth().post(EndPoints.operations_read);
    }
